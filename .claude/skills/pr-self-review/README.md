...
- invariants → applied migrations, lockfiles, vendored contracts, secrets,
  INJECTION_GUARD (root `AGENTS.md` "Do not touch" + hard rules)
...
| Deterministic verdict (table, not prose) | `gateTriggered()` / `countBlockers()` in `reviewer-core/src/output/to-review.ts`; root AGENTS.md golden rule "the model's self-reported score is never trusted" |
...
| Skill-to-diff classification | `.claude/skills/README.md` catalog scopes + module AGENTS.md files |

## Enforcement — skill-level only, by design

The gate is advisory-but-firm: on BLOCK the skill refuses to assist opening,
pushing, or merging, and the root `AGENTS.md` golden rule makes running it a
