...
| `mcp/`           | `@devdigest/mcp`            | MCP server (stdio): five tools for coding agents      | — (outbound only) |
...
- **Skills Lab** — reusable markdown instruction blocks bound to agents and injected into their review prompt, with import (.md/.zip), versioning, and per-run prompt-trace visibility.
...
| mcp (MCP tools, hermetic) | `mcp.yml` | no |
