...
describe("ConventionCard", () => {
  it("shows the rule, its category, the file:line, the verified snippet and confidence", () => {
    renderCard();
  ...
  it("shows the code-counted matching-files figure next to the confidence bar", () => {
    renderCard();
    // Rendered as "· 4 matching files" beside the confidence percentage.
    expect(screen.getByText(/4 matching files/)).toBeInTheDocument();
  });

  it("omits the occurrences figure when the rule had no identifying token", () => {
    renderCard({ occurrences: null });
    expect(screen.queryByText(/matching files/)).not.toBeInTheDocument();
  });
  ...
  it("edits the rule inline and saves the trimmed text", () => {
