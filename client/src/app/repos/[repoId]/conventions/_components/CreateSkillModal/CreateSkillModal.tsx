...
export function CreateSkillModal({
  ...
  // Pre-select the first enabled agent: the point of the flow is a skill that
  // is immediately linked to a reviewer, and the default path should not
  // require an extra click. Ref-guarded so a manual "None" is respected.
  const preselected = React.useRef(false);
  React.useEffect(() => {
    if (preselected.current || !agents?.length) return;
