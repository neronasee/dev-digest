...
describe("ConventionsPage", () => {
  ...
  it("swaps the header action to Re-scan once a board exists", () => {
    // Before any scan: two "Run scan" entry points (header + empty state), no Re-scan.
    renderPage();
    expect(screen.getAllByText("Run scan")).toHaveLength(2);
    expect(screen.queryByText("Re-scan")).not.toBeInTheDocument();

    // With a board (the extract response seeds the list cache in the real
    // hook), the header action becomes Re-scan and Run scan is gone entirely.
    cleanup();
    candidates = [candidate("a", "pending")];
    renderPage();
  ...
  it("shows no Create skill action until something is accepted", () => {
    candidates = [candidate("a", "pending")];
    renderPage();
