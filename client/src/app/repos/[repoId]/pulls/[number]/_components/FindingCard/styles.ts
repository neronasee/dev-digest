...
export const s = {
  card: (focused: boolean, sevColor: string, muted: boolean): CSSProperties => {
    const edge = focused ? sevColor : "var(--border)";
    return {
      borderRadius: 8,
      // Fully longhand: `borderColor`/`borderWidth` are shorthands too — mixing
      // them with `borderLeft*` longhands trips React 19's conflicting-style warning.
      borderStyle: "solid",
      borderTopWidth: 1,
      borderRightWidth: 1,
      borderBottomWidth: 1,
