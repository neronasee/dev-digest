/* PR list view — the body of /repos/:repoId/pulls. Ported from
   screen_dashboard.jsx; fetches GET /repos/:id/pulls (F1). Filters/sort live
   in query (?status&sort), read here via useSearchParams so the route page
   stays a thin server shell (F15). */
"use client";
...
export function PullsListView() {
  ...
  // Sort shares the URL with status (?status&sort); only the free-text query
  // stays local — typing would rewrite the URL on every keystroke.
