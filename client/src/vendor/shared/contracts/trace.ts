...
export const PromptAssembly = z.object({
  ...
  /** Composed PR-intent block; null when absent. */
...
export const RunStats = z.object({
  ...
  // USD cost of the run; null when the model's price is unknown (never a fake 0).
...
export const RunSummary = z.object({
  ...
  // USD cost of the run; null when the model's price is unknown or the run
  // failed before a completion was billed.
  cost_usd: z.number().nullable(),
