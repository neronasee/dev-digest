 ...
 * decides which of its candidates survive, the occurrence token that feeds the
 * code-side frequency count, and the skill markdown assembled from the
 * accepted ones. No IO, no model, no DB.
 */
...
/**
 * Directory-diversity picks (quick-win: sample by layer, not only by rank).
 * Walk the ranked pool in order and keep a file whose directory nothing in the
 * existing sample covers — one file per unseen directory, at most `maxPicks`.
 */
export function pickDirectoryDiverse(
  pool: string[],
  already: string[],
  maxPicks: number,
): string[] {
  const dirOf = (p: string) => p.slice(0, p.lastIndexOf('/'));
  const seen = new Set(already.map(dirOf));
  const picks: string[] = [];
  for (const p of pool) {
...
export interface VerifiedCandidate {
  ...
  confidence: number;
  /** Code-counted files sharing this rule's distinctive token (set by the
   *  service after the gate; null when no token identifies the rule). */
...
// ---- occurrence counting (quick-win: real frequency, not self-report) ------

/** Escape a literal for use inside a regex (`CodeIndex.grep` takes a pattern). */
export function escapeRegExp(s: string): string {
  return s.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
}

/**
 * The most distinctive identifier in a verified snippet: the longest token
 * that is not a keyword every file shares. `getOccurrences` greps the whole
 * clone for it and counts DISTINCT files — a number the model cannot fake.
 * Returns null when nothing in the snippet identifies the rule.
 */
export function distinctiveToken(snippet: string): string | null {
  const tokens = snippet
    .split(/[^A-Za-z0-9_$]+/)
    .filter((t) => t.length >= MIN_TOKEN_CHARS && !COMMON_TOKEN_DENYLIST.has(t.toLowerCase()));
...
/** `acme/payments-api` → `payments-api`; used for the skill body's heading. */
 ...
 * concrete shape to compare a diff against. The skill NAME is the fixed
 * `repo-conventions` (the repo it came from stays in the description and the
 * body heading, so a second repo's skill is told apart after a rename).
 */
