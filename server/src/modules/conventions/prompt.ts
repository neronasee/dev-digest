...
export const ExtractionSchema = z.object({
  candidates: z.array(
    z.object({
       ...
       * count is not evidence. The persisted `occurrences` is counted in code
       * with ripgrep after the gate (see helpers.distinctiveToken).
       */
...
You are given a code-selected sample of the repository: its configuration files, its most central source files, a spread of files from directories the central set misses, and some test files — each listed with 1-based line numbers in a left gutter.
...
export function buildUserPrompt(
  ...
  return [
    ...
    ...(rejectedRules.length > 0
      ? [
          'ALREADY DISMISSED — the maintainer rejected these rules in earlier scans.',
          'Do not propose them again, and do not propose near-variants of them:',
          ...rejectedRules.map((r) => `- ${r}`),
          '',
        ]
      : []),
    'SAMPLE',
