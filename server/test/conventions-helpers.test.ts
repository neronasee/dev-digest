 ...
 * evidence gate that decides which of its candidates survive, and the
 * occurrence token / diversity picks that feed the quality upgrades.
 * Hermetic — no DB, no model, no filesystem.
 */
...
describe('verifyCandidate', () => {
  it('keeps a candidate whose snippet is really in the cited file', () => {
    ...
    expect(result.candidate.occurrences).toBeNull(); // counted later, by the service
  });
...
describe('escapeRegExp / distinctiveToken', () => {
  it('escapes regex metacharacters so a token greps as a literal', () => {
    expect(escapeRegExp('a.b(c)')).toBe('a\\.b\\(c\\)');
  });

  it('picks the longest non-keyword identifier from a snippet', () => {
    expect(distinctiveToken('const user = await findUserById(id);')).toBe('findUserById');
  });

  it('skips the keywords every file shares', () => {
    // export/default/async/function are all denylisted — the identifier wins.
    expect(distinctiveToken('export default async function parseJwt() {')).toBe('parseJwt');
  });

  it('returns null when nothing identifies the rule', () => {
    expect(distinctiveToken('const x = await foo;')).toBeNull(); // all tokens < 5 chars or keywords
  });
});

describe('pickDirectoryDiverse', () => {
  it('keeps one file per not-yet-seen directory, in rank order', () => {
    const pool = [
      'src/api/users.ts',
      'src/api/orders.ts', // same dir as the first — skipped
      'src/db/repo.ts',
      'src/lib/helpers.ts',
    ];
    const picks = pickDirectoryDiverse(pool, ['src/app/page.tsx'], 4);
    expect(picks).toEqual(['src/api/users.ts', 'src/db/repo.ts', 'src/lib/helpers.ts']);
  });

  it('never picks a path the core sample already covers', () => {
    const picks = pickDirectoryDiverse(['src/api/users.ts'], ['src/api/other.ts'], 4);
    expect(picks).toEqual([]);
  });

  it('respects the cap', () => {
    const pool = ['a/1.ts', 'b/2.ts', 'c/3.ts'];
    expect(pickDirectoryDiverse(pool, [], 2)).toEqual(['a/1.ts', 'b/2.ts']);
  });
});
...
describe('buildSkillDraft', () => {
  ...
  it('names the skill repo-conventions, carrying the repo in the description', () => {
  ...
  it('keeps the repo slug in the body heading, so two repos stay told apart', () => {
    const draft = buildSkillDraft('acme/payments-api', [row()]);
    expect(draft.body).toContain('# payments-api-conventions');
    expect(draft.body).toContain('acme/payments-api');
  });
